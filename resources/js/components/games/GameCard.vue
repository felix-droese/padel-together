        ...
        <div v-if="props.game.result" class="mt-6">
            <div class="w-fit rounded-lg border bg-card p-4 text-sm shadow-sm">
                <div class="grid grid-cols-[120px_40px_40px_40px] gap-x-4 gap-y-1">
                    <div class="col-span-1 font-medium text-black text-sm">Result</div>
                    <div v-for="i in 3" :key="i" class="text-center text-xs font-medium uppercase tracking-wider text-muted-foreground">
                        Set {{ i }}
                    </div>

                    <div class="font-medium mt-4" :class="{ 'text-green-600': props.game.winning_team?.id === props.game.first_team.id }">First Team</div>
                    <div
